 ...
 */
 public final function defineWithGenericFilter($name, $filter, $options=[], $flags=0){
	...
	// /**
	//  * Define a new parameter. Parameter value must be an array
	//  * @param string $name Parameter name
	//  * @param string[] $options Options : 'default', 'dimension'.
	//  * @return string|null Elected value (sanitized) if accepted; false if value is rejected; null if parameter is missing and there is no default value.
	//  */
	// filter_input(METHOD,$tabFig,FILTER_SANITIZE_STRING, ['flags'=>FILTER_REQUIRE_ARRAY])
	// public final function defineArray($name,$options=[]){
	// 	$option['flag'] =
	// 				function($v){$v =filter_var($v,FILTER_SANITIZE_STRING);return } 
	// 	return $this->defineWithGenericFilter($name,FILTER_SANITIZE_STRING,$options,FILTER_REQUIRE_ARRAY);
	// }
	/**
